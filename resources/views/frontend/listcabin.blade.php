    ...
    <section class="cabin-list-container">
        @forelse ($cabins as $cabin)
            @php
                ...
                // Ambil semua foto room untuk preview
                $allRoomPhotos = [];
                foreach($cabin->rooms as $room) {
                    if(is_array($room->room_photos) && !empty($room->room_photos)) {
                        $allRoomPhotos = array_merge($allRoomPhotos, $room->room_photos);
                    }
                }
                $allRoomPhotos = array_unique(array_slice($allRoomPhotos, 0, 5)); // Maksimal 5 foto
            @endphp
                    ...
                    <div class="cabin-list-item-content">
                        ...
                        @if($cheapestRoom)
                            <div class="cabin-meta">
                                ...
                                <span><i class="fas fa-bed"></i> {{ $cheapestRoom->slot_room }} Kamar</span>
                            </div>
                        @endif
                        
                        <p class="cabin-description">{{ \Illuminate\Support\Str::limit($cabin->description, 120, '...') }}</p>
                        
                        @if(!empty($allRoomPhotos))
                            <div class="room-preview">
                                ...
                                <div class="room-photos-container">
                                    @foreach($allRoomPhotos as $photo)
